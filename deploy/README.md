...
При смене версии правил начальный пример пересчитывается. Сохранённые расчёты
прежней версии требуют повторной загрузки исходных файлов; старые CSV остаются
в каталоге запуска до обычной очистки. Не выдавайте старые результаты за новый расчёт.
