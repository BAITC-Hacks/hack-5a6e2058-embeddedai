...
export function timeline(node: NodeDetail): string {
  ...
  const first = Date.parse(t.daily[0].date), last = Date.parse(t.daily.at(-1)?.date ?? "");
  const span = Math.max(1, (last - first) / 86400000 + 1);
  const max = Math.max(...t.daily.flatMap(d => [d.in_kzt, d.out_kzt]), 1);
  const width = 620, step = width / span, bar = Math.max(1, Math.min(9, step / 3));
  const bars = t.daily.map(day => {
    const x = 30 + ((Date.parse(day.date) - first) / 86400000 + 0.5) * step;
    const a = day.in_kzt / max * 85, b = day.out_kzt / max * 85;
...
export async function openInvestigation(run: string, node: NodeDetail, select: (gid: string) => Promise<void>) {
