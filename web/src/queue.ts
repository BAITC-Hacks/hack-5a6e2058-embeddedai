...
export class InvestigationQueue {
  ...
  constructor(
    ...
    $("queue-view").innerHTML = `<div class="workspace-intro"><p class="eyebrow">РАБОЧИЙ СПИСОК АНАЛИТИКА</p><h2>Очередь проверки</h2><p>Все узлы по фильтрам слева, включая те, которых нет в TOP 50. Приоритет и место рассчитаны по всему набору.</p></div>
      <div class="queue-controls"><label>Поиск по части gid<input id="queue-search" placeholder="Часть идентификатора" autocomplete="off"></label><label>Сортировка<select id="queue-sort"><option value="rank">Приоритет проверки</option><option value="volume">Денежный объём</option><option value="in_kzt">Входящая сумма</option><option value="out_kzt">Исходящая сумма</option><option value="in_deg">Число плательщиков</option><option value="out_deg">Число получателей</option><option value="role_score">Поддержка роли</option></select></label><label>Порядок<select id="queue-order"><option value="asc">По возрастанию</option><option value="desc">По убыванию</option></select></label></div>
      <div class="queue-selection"><span id="queue-selected-count"></span><button id="queue-export" class="text-button" disabled>CSV выбранных ↓</button><button id="queue-clear" class="text-button" disabled>Очистить выбор</button><button id="queue-ask" class="text-button" disabled>Спросить AI →</button></div><p class="fine-print">До 20 узлов в одном вопросе. Выбор и заметки сохраняются только в этом браузере для текущего расчёта; заметки не являются доказательствами и не отправляются AI.</p><p id="queue-storage" class="fine-print" hidden>Хранилище браузера недоступно: выбор и заметки сохранятся только до перезагрузки.</p><div id="queue-error" class="alert" role="alert" hidden></div><div id="queue-table" class="table-scroll" aria-live="polite"></div><div class="queue-pagination"><button id="queue-prev" class="button secondary" disabled>← Назад</button><span id="queue-count" role="status"></span><button id="queue-next" class="button secondary" disabled>Далее →</button></div>`;
      ...
      if (state && typeof state === "object") {
        if (Array.isArray(state.selected))
          this.selected = new Set(
            state.selected.filter((gid: unknown) => typeof gid === "string" && /^-?\d+$/.test(gid)).slice(0, selectionLimit),
          );
        if (state.notes && typeof state.notes === "object") {
          for (const [gid, note] of Object.entries(state.notes))
            if (/^-?\d+$/.test(gid) && typeof note === "string") this.notes[gid] = note.slice(0, 1000);
        }
      }
    } catch {
  ...
  private persist() {
    try {
      localStorage.setItem(`money-graph-queue-${this.run}`, JSON.stringify({ selected: [...this.selected], notes: this.notes }));
    } catch {
      $("queue-storage").hidden = false;
    }
  }
  private syncSelection() {
    ...
    for (const checkbox of $("queue-table").querySelectorAll<HTMLInputElement>("[data-queue-check]")) {
  ...
  private async load() {
    ...
    try {
      ...
      for (const input of $("queue-table").querySelectorAll<HTMLTextAreaElement>("[data-queue-note]"))
        input.oninput = () => {
      ...
      if (sequence !== this.sequence) return;
      $("queue-error").textContent = error instanceof Error ? error.message : "Не удалось открыть очередь";
      $("queue-error").hidden = false;
      $("queue-table").innerHTML = '<button id="queue-retry" class="button secondary">Повторить загрузку</button>';
  ...
  private async export() {
    ...
    try {
      const nodes = await Promise.all(gids.map((gid) => api<NodeDetail>(`/api/runs/${run}/nodes/${encodeURIComponent(gid)}`)));
      if (run !== this.run) return;
      const header = ["gid_text", "role", "priority_score", "rank", "cluster_id", "in_kzt", "out_kzt", "evidence", "analyst_note"];
      const lines = nodes.map((node) => [csvCell(node.gid, true), ...[node.role, node.priority_score, node.rank, node.cluster_id, node.in_kzt, node.out_kzt, node.evidence, this.notes[node.gid] ?? ""].map((cell) => csvCell(cell))].join(","));
      download(`investigation-${run}.csv`, `\uFEFF${header.join(",")}\r\n${lines.join("\r\n")}\r\n`, "text/csv;charset=utf-8");
    } catch (error) {
      if (run === this.run) {
        $("queue-error").textContent = error instanceof Error ? error.message : "Не удалось выгрузить выбор";
        $("queue-error").hidden = false;
      }
    } finally {
