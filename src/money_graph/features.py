...
def compute(graph: nx.DiGraph, nodes: pd.DataFrame, rules: dict[str, Any]) -> pd.DataFrame:
    ...
    for column, metric in {
        ...
        "betweenness": nx.betweenness_centrality(
            graph, k=min(rules["betweenness_samples"], len(graph)),
            seed=rules["random_seed"], weight=None,
        ),
    }.items():
        frame[column] = pd.Series(metric)
    reach = dict.fromkeys(graph, 0)
    for seed in nodes.loc[nodes.is_seed, "gid"]:
        for gid in nx.single_source_shortest_path_length(graph, int(seed), cutoff=rules["max_depth"]):
            if gid != seed:
                reach[gid] += 1
    ...
    for metric in ("pagerank", "betweenness", "seed_reach", "in_deg", "out_deg", "in_tx", "out_tx", "out_kzt"):
...
def communities(graph: nx.DiGraph, rules: dict[str, Any]) -> dict[int, int]:
    ...
    groups = list(nx.community.louvain_communities(
        connected, weight="weight", seed=rules["random_seed"], resolution=rules["louvain_resolution"],
    )) if len(connected) else []
