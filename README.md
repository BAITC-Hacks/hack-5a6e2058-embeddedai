...
Версия приложения — **0.6.0**, правила — **2.2**.
...
- AI-ассистент: вопрос на русском → вычисленный ответ по графу → ссылки на карточки.
  Поддерживает общих получателей/плательщиков, пути, роли, рейтинг и сообщества.
...
[аудит каждого пункта ТЗ](docs/AUDIT_06.md), [проверки 0.6](docs/VALIDATION_06.md).
...
flowchart LR
    ...
    Q[Вопрос + выбранные gid] --> L[OpenAI: план запроса]
    L --> D[Проверка плана + вычисление по графу]
    A --> D
    D --> U
    X[Caddy / HTTPS] --> A
```
...
- AI понимает ограниченный набор запросов; числа, пути и ссылки вычисляет сервер.
  Произвольные запросы по датам и суммы сквозного движения средств не поддерживаются.
  Интерпретация вопроса моделью может ошибаться: интерфейс показывает выполненный
  запрос и ограничения. Автоматические решения о блокировке счетов не принимаются.
...
| `OPENAI_MODEL` | `gpt-4.1-mini-2025-04-14` | Модель для интерпретации вопроса |
...
Выберите узлы в очереди, откройте «AI-ассистент» и спросите, например:
«Кто собирает деньги с этих пятерых?» или «Кто получает от них за два шага?».
Ссылки в ответе открывают реальные карточки; отсутствие общего получателя
показывается как отсутствие результата. Вопрос можно задать и с точными gid.

В OpenAI отправляются **только вопрос и выбранные gid**. Сырые таблицы, потоки,
заметки и API-ключ в сообщении модели отсутствуют. Модель формирует ограниченный
план, сервер проверяет его и сам вычисляет ответ. Для запроса используется
`store: false`; это не заменяет политики обработки данных провайдера.
...
не сохраняет код. Ограничение — один AI-запрос одновременно, не более 60 в час
на процесс, тайм-аут сетевого ожидания 30 секунд. [Устройство и примеры](docs/ASSISTANT.md).
