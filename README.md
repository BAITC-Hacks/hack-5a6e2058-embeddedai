...
Версия приложения — **0.4.0**, правила — **2.1**.
...
репозитория и выполните одинаковую команду на всех трёх ОС:
...
[VALIDATION_04.md](docs/VALIDATION_04.md).
...
Код завершения **0** означает успешную запись файлов. Некорректные аргументы,
...
В репозитории настроена [CI-матрица](.github/workflows/verify.yml): backend на
Linux, Windows и macOS; полный frontend/browser workflow на Linux; отдельный
запуск трёх входных файлов и сравнение CSV с эталонными `artifacts/`.
Результат запуска CI указывается в [VALIDATION_04.md](docs/VALIDATION_04.md).
...
[аудит ТЗ](docs/AUDIT.md), [проверки 0.4](docs/VALIDATION_04.md),
[предыдущий подробный аудит 0.3](docs/VALIDATION.md).
